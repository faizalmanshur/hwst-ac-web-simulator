...
function App() {
  ...
  return (
    <main className="app-shell">
      {page !== 'visualisasi' && (<section className="hero">
        <div>
          ...
          <nav className="page-nav">
            ...
            <button onClick={() => setPage('visualisasi')}>Visualisasi Sistem</button>
...
function SystemJourneyPage({ results, onRun, loading, setPage, onDownload }: { results: SimulationResults | null; onRun: () => void; loading: boolean; setPage: (p: Page) => void; onDownload: (type: 'excel' | 'pdf') => void }) {
  ...
  return (
    <section className="vis-reference-shell">
      <aside className="vis-sidebar">
        <button className="vis-side-menu" aria-label="Menu">☰</button>
        <nav className="vis-side-nav" aria-label="Navigasi visualisasi">
          <button title="Dashboard" onClick={() => setPage('dashboard')}>▥</button>
          <button title="Visualisasi Sistem" className="active" onClick={() => setPage('visualisasi')}>⌁</button>
          <button title="Diagram P-h" onClick={() => setPage('ph')}>▤</button>
          <button title="Pengaturan" onClick={() => setPage('dashboard')}>⚙</button>
        </nav>
        <div className="vis-side-bottom">
          <button title="Mode gelap">◐</button>
      ...
      <div className="vis-reference-content">
        <header className="vis-page-header">
          ...
          <div className="vis-header-actions">
            ...
            <button className="vis-download-btn" onClick={() => onDownload('pdf')} disabled={!results}>⇩ <span>Unduh Laporan</span><b>⌄</b></button>
          </div>
        </header>
...
function SystemSvg({ row }: { row: TimeRow }) {
  ...
  return <div className="sys-diagram-wrap"><svg viewBox="0 0 880 440" className="sys-diagram-svg">
    ...
    {/* === PIPES === */}
    {/* 1→2: Evap out → Comp (bottom of comp, left side, up) */}
    <PipeNew d="M110 295 L110 340 L310 340 L310 290" color={cEvapOut} />
    ...
    <StateLabelNew label="1" desc="Keluar evaporator" temp={tEvapOut} x={112} y={374} color={cEvapOut} anchor="center" />
...
function PipeNew({ d, color }: { d: string; color: string }) {
  return <g>
    ...
    <path d={d} stroke="rgba(255,255,255,.85)" strokeWidth="2.5" fill="none" strokeDasharray="4 16" strokeLinecap="round" className="flow-dash" />
  </g>
}
...
function LineChartActive({ data, xKey, yKey, title, yLabel, activeIdx, yAxisLabel }: { data: TimeRow[]; xKey: string; yKey: string; title: string; yLabel: string; activeIdx: number; yAxisLabel: string }) {
  ...
  return <div className="chart-card chart-card-v9">
    ...
    <svg viewBox={`0 0 ${w} ${h}`}>
      ...
      <text className="axis-label-x" x={(padL+w-padR)/2} y={h-8} textAnchor="middle">Waktu (jam)</text>
    </svg>
