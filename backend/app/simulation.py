...
def validate_config(cfg: dict[str, Any]) -> None:
    required_positive = [
        ...
        "cond_airflow_m3_s", "k_tube_cond_W_mK", "h_air_cond_W_m2K",
        ...
        "evap_tube_row_pitch_mm", "evap_return_bend_factor", "evap_airflow_m3_s",
...
def compute_u_zones(cfg: dict[str, Any], prop: dict[str, Any], geom: dict[str, Any], mdot: float) -> dict[str, Any]:
    ...
    try:
        ...
        h_cond_dsh = htc_single_phase(cfg, prop, mdot, geom["cond"]["D_i_m"], "high_vapor_cooling")
        h_cond_sc = htc_single_phase(cfg, prop, mdot, geom["cond"]["D_i_m"], "high_liquid_cooling")
        h_evap_sh = htc_single_phase(cfg, prop, mdot, geom["evap"]["D_i_m"], "low_vapor_heating")
        h_hw_tp = htc_shah_condensation(cfg, prop, mdot, geom["hwst"]["D_i_m"])
        h_cond_tp = htc_shah_condensation(cfg, prop, mdot, geom["cond"]["D_i_m"])
        h_evap_tp = htc_shah_evaporation(cfg, prop, mdot, geom["evap"]["D_i_m"], nominal_heat_flux(cfg, geom["evap"]["A_i_m2"]))

        def calc(comp: str, zone: str, g: dict[str, Any], hi: float, ho: float, ktube: float, eta: float, rf: float, rc: float) -> tuple[float, dict[str, Any]]:
            uo, ua = u_from_resistance(g["A_i_m2"], g["A_o_m2"], g["D_i_m"], g["D_o_m"], g["tube_length_m"], hi, ho, ktube, eta, rf, rc)
            return uo, {"komponen": comp, "zona": zone, "h_ref_W_m2K": round(hi, 3), "h_air_water_W_m2K": round(ho, 3), "U_final_W_m2K": round(uo, 3), "UA_final_kW_K": round(ua, 5)}

        rows = []
        vals = []
        for key, comp, zone, gkey, hi, ho, ktube, eta, rf, rc in [
            ("U_hwst_dsh_W_m2K", "HWST", "DSH", "hwst", h_hw_dsh, _num(cfg, "h_water_hwst_W_m2K"), _num(cfg, "k_tube_hwst_W_mK"), 1.0, _num(cfg, "Rf_hwst_m2K_W"), 0),
            ("U_hwst_tp_W_m2K", "HWST", "TP", "hwst", h_hw_tp, _num(cfg, "h_water_hwst_W_m2K"), _num(cfg, "k_tube_hwst_W_mK"), 1.0, _num(cfg, "Rf_hwst_m2K_W"), 0),
            ("U_hwst_sc_W_m2K", "HWST", "SC", "hwst", h_hw_sc, _num(cfg, "h_water_hwst_W_m2K"), _num(cfg, "k_tube_hwst_W_mK"), 1.0, _num(cfg, "Rf_hwst_m2K_W"), 0),
            ("U_cond_dsh_W_m2K", "Kondensor", "DSH", "cond", h_cond_dsh, _num(cfg, "h_air_cond_W_m2K"), _num(cfg, "k_tube_cond_W_mK"), 1.0, _num(cfg, "Rf_cond_m2K_W"), 0),
            ("U_cond_tp_W_m2K", "Kondensor", "TP", "cond", h_cond_tp, _num(cfg, "h_air_cond_W_m2K"), _num(cfg, "k_tube_cond_W_mK"), 1.0, _num(cfg, "Rf_cond_m2K_W"), 0),
            ("U_cond_sc_W_m2K", "Kondensor", "SC", "cond", h_cond_sc, _num(cfg, "h_air_cond_W_m2K"), _num(cfg, "k_tube_cond_W_mK"), 1.0, _num(cfg, "Rf_cond_m2K_W"), 0),
            ("U_evap_tp_W_m2K", "Evaporator", "TP", "evap", h_evap_tp, _num(cfg, "h_air_evap_W_m2K"), _num(cfg, "k_tube_evap_W_mK"), 1.0, _num(cfg, "Rf_evap_m2K_W"), 0),
            ("U_evap_sh_W_m2K", "Evaporator", "SH", "evap", h_evap_sh, _num(cfg, "h_air_evap_W_m2K"), _num(cfg, "k_tube_evap_W_mK"), 1.0, _num(cfg, "Rf_evap_m2K_W"), 0),
        ]:
            u, row = calc(comp, zone, geom[gkey], hi, ho, ktube, eta, rf, rc)
            U[key] = u
            rows.append(row)
            vals.append(u)
        U["description"] = "Thermal resistance + auto h_refrigerant + Shah TP correlations"
        U["rows"] = rows
    except Exception as exc:
        ...
        if t_in > ttop + 1e-4:
            if tc > ttop + 1e-4:
                ...
                if a_need <= a_rem:
                    ...
                    h_out = hg + cp_dsh * max(0, t_out - tc)
                    q = mdot * (h - h_out)
                    h = h_out
            else:
                a_dsh = a_rem
...
def evaporator_model_manual_shr(cfg: dict[str, Any], prop: dict[str, Any], geom: dict[str, Any], mdot: float, h_in: float, u: dict[str, Any]) -> dict[str, Any]:
    ...
    q_sens = shr * q_total
    q_lat = max(0, (1 - shr) * q_total)
    h_fg_water = max(2250, 2501 - 2.381 * prop["T_evap_C"])
    return {
        "Q_kW": q_total,
        "h_out_kJ_kg": h,
        "T_air_out_C": tair,
...
def two_phase_dp_kpa(mdot: float, length: float, D: float, rho_g: float, rho_l: float, mu_g: float, mu_l: float, x: float, rough: float) -> float:
    ...
    rho_mix = 1 / max(x / max(rho_g, EPS) + (1 - x) / max(rho_l, EPS), EPS)
    mu_mix = x * mu_g + (1 - x) * mu_l
    return single_phase_dp_kpa(mdot, length, D, rho_mix, mu_mix, rough) * (1 + 12 * x * (1 - x))
...
def estimate_zone_pressure_drops(cfg: dict[str, Any], prop: dict[str, Any], geom: dict[str, Any], mdot: float, hwst: dict[str, Any], cond: dict[str, Any], evap: dict[str, Any]) -> dict[str, Any]:
    if int(round(_num(cfg, "pressure_drop_model", 2))) == 0:
        return {"DP_hwst_kPa": 0.0, "DP_cond_kPa": 0.0, "DP_evap_kPa": 0.0, "DP_high_side_kPa": 0.0, "DP_low_side_kPa": 0.0}
    ...
    L_co = max(geom["cond"]["tube_length_m"], 0.01)
    L_ev = max(geom["evap"]["tube_length_m"], 0.01)
    ...
    mu_g, mu_l = 1.3e-5, 1.8e-4
    dp_hw = single_phase_dp_kpa(mdot, L_hw * hwst["f_dsh"], D_hw, rho_g_hi, mu_g, rough) + two_phase_dp_kpa(mdot, L_hw * hwst["f_tp"], D_hw, rho_g_hi, rho_l_hi, mu_g, mu_l, 0.5, rough) + single_phase_dp_kpa(mdot, L_hw * hwst["f_sc"], D_hw, rho_l_hi, mu_l, rough)
    dp_co = single_phase_dp_kpa(mdot, L_co * cond["f_dsh"], D_co, rho_g_hi, mu_g, rough) + two_phase_dp_kpa(mdot, L_co * cond["f_tp"], D_co, rho_g_hi, rho_l_hi, mu_g, mu_l, 0.5, rough) + single_phase_dp_kpa(mdot, L_co * cond["f_sc"], D_co, rho_l_hi, mu_l, rough)
    dp_ev = two_phase_dp_kpa(mdot, L_ev * evap["f_tp"], D_ev, rho_g_lo, rho_l_lo, mu_g, mu_l, 0.5, rough) + single_phase_dp_kpa(mdot, L_ev * evap["f_sh"], D_ev, rho_g_lo, mu_g, rough)
    return {"DP_hwst_kPa": max(0, dp_hw), "DP_cond_kPa": max(0, dp_co), "DP_evap_kPa": max(0, dp_ev), "DP_high_side_kPa": max(0, dp_hw + dp_co), "DP_low_side_kPa": max(0, dp_ev)}
...
def capillary_diagnostic_model(cfg: dict[str, Any], prop: dict[str, Any], h_in_cap: float, mdot_comp: float) -> dict[str, Any]:
    if int(round(_num(cfg, "capillary_diagnostic_mode", 1))) == 0:
        return {"model": "OFF", "mdot_cap_kg_s": None, "error_pct": None, "status_code": 0, "status_text": "OFF", "DP_available_kPa": None, "DP_required_for_mdot_comp_kPa": None, "Re": None, "x_in_capillary": None}
    D = max(_num(cfg, "capillary_D_i_mm") / 1000, 1e-6)
    L = max(_num(cfg, "capillary_length_m"), 1e-6)
    ...
    if not math.isfinite(mdot) or mdot <= 0:
        code, text = 0, "INVALID"
    elif xin > 0.02:
        code, text = 4, "INLET_NOT_SUBCOOLED"
    elif abs(err) <= 5:
        code, text = 1, "BALANCED_CLOSE"
    elif abs(err) <= 15:
        code, text = 5, "WITHIN_TOLERANCE"
    elif err < -15:
        code, text = 2, "CAPILLARY_UNDERFEED"
    else:
        code, text = 3, "CAPILLARY_OVERFEED"
    return {"model": "Darcy-Weisbach liquid-only capillary diagnostic/feedback", "mdot_cap_kg_s": mdot, "error_pct": err, "status_code": code, "status_text": text, "DP_available_kPa": dp / 1000, "DP_required_for_mdot_comp_kPa": dp_req, "Re": re, "x_in_capillary": xin}
...
def simulate_single_volume(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    for step in range(max_steps):
        ...
        dp_feedback = int(round(_num(cfg, "pressure_drop_feedback", 1))) == 1
        n_feedback = 1 + 2 * int(cap_feedback or dp_feedback)
        ...
        for fb in range(n_feedback):
            ...
            if cap_feedback and cap_diag.get("mdot_cap_kg_s") and cap_diag["mdot_cap_kg_s"] > 0:
            ...
            if fb < n_feedback - 1:
                ...
                if cap_feedback and cap_diag.get("DP_required_for_mdot_comp_kPa") and cap_diag["DP_required_for_mdot_comp_kPa"] > 0:
...
def assemble_results(cfg: dict[str, Any], prop: dict[str, Any], geom: dict[str, Any], comp: dict[str, Any], rows: list[dict[str, Any]], status: str, reach_time_min: float | None, w_comp_initial: float) -> dict[str, Any]:
    ...
    return {
        ...
        "volume_variation": build_volume_variation(cfg),
        "config": cfg,
        "engine_info": {
            "engine_version": "V9 MATLAB latest compressor calibration + compact professional visualization + complete web report",
...
def build_visualization_series(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    keys = [
        ...
        "capillary_status_code", "capillary_status",
    ]
...
def simulate_conventional_baseline(cfg: dict[str, Any], prop: dict[str, Any], geom: dict[str, Any], operation_min: float) -> dict[str, Any]:
    ...
    p = dict(prop)
    p["h_comp_out_kJ_kg"] = comp["h5_kJ_kg"]
    p["T_comp_out_C_model"] = comp["T5_C"]
    u = compute_u_zones(cfg, p, geom, comp["mdot_kg_s"])
    cond = condenser_model(cfg, p, geom, comp["mdot_kg_s"], p["h_comp_out_kJ_kg"], u)
    evap = evaporator_model_manual_shr(cfg, p, geom, comp["mdot_kg_s"], cond["h_out_kJ_kg"], u)
    return {
        "COP_AC": evap["Q_kW"] / max(comp["Pcomp_kW"], EPS),
        "COP_sensible": evap["Q_sensible_kW"] / max(comp["Pcomp_kW"], EPS),
        "Q_evap_kW": evap["Q_kW"],
        "Q_cond_kW": cond["Q_kW"],
        "W_comp_kW": comp["Pcomp_kW"],
        "energy_evap_kWh": evap["Q_kW"] * operation_min / 60,
        "energy_comp_kWh": comp["Pcomp_kW"] * operation_min / 60,
        "h_evap_out_kJ_kg": evap["h_out_kJ_kg"],
        "T_evap_out_C": evap["T_out_C"],
        ...
        "T_cond_out_C": cond["T_out_C"],
        "h_exp_out_kJ_kg": cond["h_out_kJ_kg"],
        "T_evap_in_C": prop["T_evap_C"],
        "P_high_kPa": prop["P_high_Pa"] / 1000.0,
        "P_low_kPa": prop["P_low_Pa"] / 1000.0,
    }
...
def build_summary_rows(cfg: dict[str, Any], prop: dict[str, Any], comp: dict[str, Any], final: dict[str, Any], best_dsh: dict[str, Any], best_overall: dict[str, Any], baseline: dict[str, Any], sv: dict[str, Any], status: str) -> list[dict[str, Any]]:
    ...
    rows = [
        ...
        ("Mode perhitungan U", "Thermal resistance + auto h_refrigerant + Shah TP correlations", "-"),
        ...
        ("SHR manual evaporator", _num(cfg, "SHR_manual"), "-"),
...
def capillary_status_text(code: Any) -> str:
    try:
        ...
        c = 0
    return {1: "BALANCED_CLOSE", 2: "CAPILLARY_UNDERFEED", 3: "CAPILLARY_OVERFEED", 4: "INLET_NOT_SUBCOOLED", 5: "WITHIN_TOLERANCE"}.get(c, "INVALID/OFF")
...
def build_state_rows(cfg: dict[str, Any], prop: dict[str, Any], comp: dict[str, Any], baseline: dict[str, Any], final: dict[str, Any]) -> list[dict[str, Any]]:
    rows = [
        ...
        ("P suction", cfg.get("P_suction_psig"), "psig"),
        ("P discharge", cfg.get("P_discharge_psig"), "psig"),
        ...
        ("mdot model", comp.get("mdot_kg_s"), "kg/s"),
...
def build_analysis_rows(prop: dict[str, Any], final: dict[str, Any], best_dsh: dict[str, Any], baseline: dict[str, Any], sv: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        {"check": "Manual SHR evaporator", "status": "INFO", "value": f"SHR={final.get('SHR'):.3f} | Q_lat={final.get('Q_latent_kW'):.3f} kW", "recommendation": "Pembagian sensible-latent hanya untuk laporan; tidak memengaruhi state refrigeran."},
        ...
        {"check": "COP AC vs baseline simulasi", "status": "OK", "value": f"HWST DSH best {sv.get('bestCOP_AC_dsh')} vs conventional {sv.get('COP_AC_conventional')}", "recommendation": "Perbandingan dengan model komponen yang sama."},
        {"check": "Diagnostik kapiler", "status": "CAUTION", "value": f"{final.get('mdot_capillary_diag_kg_s'):.5f} / {final.get('mdot_ref_kg_s'):.5f}", "recommendation": f"Status: {capillary_status_text(final.get('capillary_status_code'))}. Jangan disebut balance sempurna."},
    ]
